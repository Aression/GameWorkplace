...
def process_videos(input_dir, output_dir, lead=10, tail=2, threshold=30, min_kills=2, progress_callback=None, state_file=None, temp_dir=None):
    """处理视频文件，跳过已处理的，并保存状态"""
    ...
    for idx, segment in enumerate(valid_segments, 1):
        ...
        for video in relevant_videos:
            ...
            # 计算视频与目标段的重叠部分
            overlap_start = max(video_start, segment.start_time)
            overlap_end = min(video_end, segment.end_time)
            
            # 如果有有效重叠，添加到时间线
            if overlap_start < overlap_end:
                timeline_segment = TimeSegment(overlap_start, overlap_end, video)
                timeline_segments.append(timeline_segment)
        
        # 合并时间线上重叠的段，得到连续的不重叠片段
        continuous_segments = merge_overlapping_segments(timeline_segments)
        
        print(f"  分析得到 {len(continuous_segments)} 个连续时间段")
        
        # 为每个连续段剪辑视频片段
        temp_clips = []
        
        for seg_idx, seg in enumerate(continuous_segments):
            # 更新剪辑进度
            if progress_callback:
                sub_progress = f"剪辑片段 {seg_idx+1}/{len(continuous_segments)}"
            ...
            print(f"  准备剪切第 {seg_idx+1}/{len(continuous_segments)} 段: {best_video['path']}")
